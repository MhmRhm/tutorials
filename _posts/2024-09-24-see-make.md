...
date: 2024-09-24
image:
  path: https://raw.githubusercontent.com/MhmRhm/tutorials/refs/heads/main/assets/img/Cmake.svg
...
*[Modern CMake for C++](https://www.packtpub.com/en-us/product/modern-cmake-for-c-9781805123361)*
...
<p align="center"><img src="https://i.postimg.cc/L87dLSGq/temp-Image-YVG4Jo.avif" alt="Dependency Graph"/></p>
...
# ... package
# ... test
# ... coverage-boost_test_see_app
# ... coverage-google_test_libsee
# ... doxygen-libsee_static
# ... doxygen-see_app_static
# ... memcheck-boost_test_see_app
# ... memcheck-google_test_libsee
# ... boost_test_see_app
# ... google_bench_libsee
# ... google_bench_see_app
# ... google_test_libsee
# ... libsee_obj
# ... libsee_shared
# ... libsee_static
...
To run the tests, use `ctest` with the appropriate preset. This command executes
all tests defined in the project:

```bash
ctest --preset linux-default-release
```
...
```json
{
	...
	"cacheVariables": {
		...
		"QTDIR": "$penv{HOME}/Qt/6.9.3/gcc_arm64"
	}
}
```
...
},
{
	...
	"targets": ["doxygen-libsee_static", "doxygen-see_app_static"]
}
]
```

The `windows-clang-debug-doxygen` preset inherits its properties from the
`windows-clang-debug` build preset and also can be marked as hidden. A hidden
preset cannot be used directly in UI tools or from the command line, but it can
still be utilized as a step in workflows.
...
This template uses vcpkg’s [manifest mode](https://learn.microsoft.com/en-us/vcpkg/concepts/manifest-mode),
allowing dependencies to be declared and managed automatically. To include
additional libraries, simply add them to the `vcpkg.json` file under the
dependencies list.
...
integrate your work into their project. See [Relocatable Packages](https://cmake.org/cmake/help/latest/prop_tgt/INTERFACE_INCLUDE_DIRECTORIES.html#prop_tgt:INTERFACE_INCLUDE_DIRECTORIES)
and [First Step](https://github.com/MhmRhm/SeeMake?tab=readme-ov-file#first-step).
...
set_target_properties(libsee_obj PROPERTIES
	PUBLIC_HEADER src/libsee/include/libsee/see.h
...
file (in this case, `see_app.cpp`) and reducing `main()` to a few function
calls or object creations, we can also test the code responsible for initializing
the application.

```cmake
add_library(see_app_static STATIC
	see_app.cpp
)
target_link_libraries(see_app_static
	PRIVATE precompiled
	PUBLIC libsee_static
)

add_executable(see main.cpp)
target_link_libraries(see
	PRIVATE precompiled
	PRIVATE see_app_static
)
```

When `see_app_static` links to `libsee_static`, all the header files in
`libsee_obj` become available to `see_app_static`. This is because of the
...
### The `Testing.cmake` File

Using CMake's CTest, you can write tests without relying on any third-party tools.
However, since more feature-rich and widely used testing frameworks are available,
...
Tests written using the Boost.Test framework can be registered automatically;
however, unlike GoogleTest, they are not registered with CTest individually.
Instead, all tests are grouped under a single entry:

```cmake
add_test(NAME "All_Boost_Tests" COMMAND ${target}) # All tests appear as one
```

Two helper macros are defined to facilitate testing while also enabling coverage
...
macro(AddBoostTests target)
	...
	add_test(NAME "${target}" COMMAND ${target}) # All tests seen as one
	AddMemcheck(${target})
endmacro()
```
...
vcpkg is used to integrate Boost into your project by automatically fetching the
library and all its submodules. While this is a one-time setup, it may take some
time to complete. Alternatively, you can install Boost separately and link it
using CMake's `find_package` feature.

In both cases, you need to specify which Boost libraries your project depends on:
...
You can add or remove libraries from this list as needed to match your project's
requirements.

### The `Protobuf.cmake` File

The core of this file is the custom function `AddProtobuf(target)`. This
...
CMake will handle the rest — generating code, building it, and linking
everything automatically.

As an example, [asio-recipes](https://github.com/MhmRhm/asio-recipes) uses this
template to demonstrate data transfer using Google Protobuf.
...
You can use it in your CMake project as follows:
